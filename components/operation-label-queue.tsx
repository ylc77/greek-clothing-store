...
export function OperationLabelQueue({ entries, total, dispatch, onPrint }: {
  entries: LabelQueueEntry[]; total: number; dispatch: (action: LabelQueueAction) => void; onPrint: () => void;
}) {
  ...
  return <details className="admin-panel my-4" open data-operation-label-queue>
    ...
    <p className="my-3 text-sm text-stone-600">份数来自本次入库，不是当前总库存。关闭打印窗口不代表打印成功；请核对实物后清空。刷新或退出将丢失此队列。</p>
    ...
    <div className="mt-3 flex flex-wrap gap-2"><button type="button" className="admin-button-primary" onClick={onPrint}>打印本次标签</button><button type="button" className="admin-button-secondary" onClick={() => { if (window.confirm("确认已打印或放弃本次标签？这不会改变库存。")) dispatch({ type: "clear" }); }}>清空队列</button></div>
  </details>;
}
