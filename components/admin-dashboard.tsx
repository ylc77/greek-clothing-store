...
const emptyProduct: ProductFormData = { sku: "", name_cn: "", name_gr: "", name_en: "", description_cn: "", description_gr: "", description_en: "", category: "men", subcategory: "tshirts", price: 0, stock: 0, sizes: "", image_url: "", image_urls: "", brand: "", barcode: "", vat: 24, color: "", skroutz_url: "", is_active: true, fit_type: "regular", material: "", ai_keywords: "", style_tags: "", size_chart: "" };
...
export function AdminDashboard() {
  ...
  function startEdit(p: AdminProduct) { setEditingId(p.id); setForm({ sku:p.sku, name_cn:p.name_cn, name_gr:p.name_gr, name_en:p.name_en, description_cn:p.description_cn, description_gr:p.description_gr, description_en:p.description_en, category:p.category, subcategory:p.subcategory, price:p.price, stock:p.stock, sizes:p.sizes, image_url:p.image_url, image_urls:p.image_urls, brand:p.brand, barcode:p.barcode, vat:p.vat, color:p.color, skroutz_url:p.skroutz_url, is_active:p.is_active, material: p.material || (p as Record<string,unknown>).material as string || "", fit_type: (p as Record<string,unknown>).fit_type as string || "regular", ai_keywords: Array.isArray((p as Record<string,unknown>).ai_keywords) ? ((p as Record<string,unknown>).ai_keywords as string[]).join(",") : String((p as Record<string,unknown>).ai_keywords || ""), style_tags: Array.isArray((p as Record<string,unknown>).style_tags) ? ((p as Record<string,unknown>).style_tags as string[]).join(",") : String((p as Record<string,unknown>).style_tags || ""), size_chart: typeof (p as Record<string,unknown>).size_chart === "object" ? JSON.stringify((p as Record<string,unknown>).size_chart) : String((p as Record<string,unknown>).size_chart || "") }); loadSizeStock(p); setShowSizeChart(!!((typeof (p as Record<string,unknown>).size_chart === "object" ? JSON.stringify((p as Record<string,unknown>).size_chart) : String((p as Record<string,unknown>).size_chart || "")).trim())); setTab("add"); window.scrollTo({ top: 0, behavior: "smooth" }); }
                ...
                <tbody>
                  {filteredProducts.slice(0, 100).map(p => (
                    <tr className="border-b border-stone-50 hover:bg-stone-50/70 transition-colors" key={p.id}>
                      ...
                      <td className="py-2 pr-3"><div className="flex flex-col gap-1"><span className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-bold whitespace-nowrap ${p.is_active ? "bg-green-100 text-green-800" : "bg-stone-100 text-stone-500"}`}>{p.is_active ? "上架" : "下架"}</span>{(() => { const hasSzChart = (p as Record<string,unknown>).size_chart && typeof (p as Record<string,unknown>).size_chart === "object" && Object.keys((p as Record<string,unknown>).size_chart as object).length > 0; const hasKeywords = Array.isArray((p as Record<string,unknown>).ai_keywords) && ((p as Record<string,unknown>).ai_keywords as unknown[]).length > 0 || typeof (p as Record<string,unknown>).ai_keywords === "string" && ((p as Record<string,unknown>).ai_keywords as string).trim().length > 0; const hasStyleTags = Array.isArray((p as Record<string,unknown>).style_tags) && ((p as Record<string,unknown>).style_tags as unknown[]).length > 0 || typeof (p as Record<string,unknown>).style_tags === "string" && ((p as Record<string,unknown>).style_tags as string).trim().length > 0; const hasMat = (p as Record<string,unknown>).material && String((p as Record<string,unknown>).material).trim(); const hasSizes = p.sizes && p.sizes.trim(); if (hasSzChart && hasKeywords && hasStyleTags && hasMat) return <span className="inline-block rounded-full px-1.5 py-0.5 text-[10px] font-bold bg-green-100 text-green-700">AI 完整</span>; const badges: string[] = []; if (!hasSzChart && !hasSizes) badges.push("缺尺码+尺寸"); else if (!hasSzChart) badges.push("缺尺码表"); if (!hasKeywords) badges.push("缺AI关键词"); if (!hasStyleTags) badges.push("缺风格标签"); if (!hasMat) badges.push("缺材质"); return badges.map((b,i) => <span key={i} className={`inline-block rounded-full px-1.5 py-0.5 text-[10px] font-bold whitespace-nowrap ${b.startsWith("缺尺码+") ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>{b}</span>); })()}</div></td>
