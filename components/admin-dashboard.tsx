...
export function AdminDashboard() {
  ...
  async function deleteProduct(p: AdminProduct) { if (!window.confirm(`下架商品 ${p.sku}？\n\n商品将被设为下架状态，前台不再显示。数据不会丢失。`)) return; setLoading(true); try { await api(`/api/admin/products/${p.id}`, { method: "DELETE" }); toast("商品已下架"); await loadProducts(); } catch (er) { toast(er instanceof Error ? er.message : "下架失败", "err"); } finally { setLoading(false); } }
