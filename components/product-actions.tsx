...
export function ProductActions({ productName, productNameEn, productNameGr, sku, sizes, sizeSystem, sizeStock, variants, stock, onlineStoreEnabled = false, aiEnabled = true, language = "el", whatsappUrl, category, subcategory, price = 0, imageUrl, sizeChart, fitType }: ProductActionsProps) {
  ...
  return (
    <div>
      ...
      <div className={`mb-5 flex items-center justify-between rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 transition ${selectionReady ? "" : "opacity-60"}`}><span className="text-sm font-bold text-stone-600">{quantityLabel}</span><div className="flex items-center gap-3"><button aria-label="Decrease quantity" className="h-10 w-10 rounded-full border border-stone-300 bg-white text-lg disabled:cursor-not-allowed" disabled={!selectionReady || quantity <= 1} onClick={() => setQuantity(value => Math.max(1, value - 1))} type="button">−</button><span className="min-w-6 text-center font-black">{quantity}</span><button aria-label="Increase quantity" className="h-10 w-10 rounded-full border border-stone-300 bg-white text-lg disabled:cursor-not-allowed" disabled={!selectionReady || quantity >= Math.min(selectedAvailable, 20)} onClick={() => setQuantity(value => Math.min(value + 1, selectedAvailable, 20))} type="button">+</button></div></div>
