...
前台可以展示商品、分类、价格、尺码和库存，并支持购物车、Viva 在线预付款、BOX NOW Locker 和到店自取；后台可以管理商品、图片、库存、CSV 和在线订单，同时也有实体店 POS、库存流水、条码标签和小票打印预览。
...
- 第一版支持 Viva 在线预付款后的 BOX NOW Locker 和到店自取。
- 后台可以查看订单，库存由事务流程安全预留和扣减。

Suggested line:

```txt
顾客可以直接在网站下单并通过 Viva Smart Checkout 预付款，商家在后台处理 BOX NOW Locker 或到店自取订单。
```
