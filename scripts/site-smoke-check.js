...
(async () => {
  ...
  await record("retired product feed", async () => {
    const response = await page.request.get(url("/feed.xml"), {
      timeout: 10000,
    });
    if (response.status() !== 410) throw new Error(`retired feed.xml status ${response.status()}, expected 410`);
  });

  await record("cart and checkout pages", async () => {
    await assertPageLoaded(page, "/cart", "main");
    await assertPageLoaded(page, "/checkout", "main");
  });
