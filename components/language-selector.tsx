...
export function LanguageSelector({ language }: { language: Language }) {
  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        className="inline-flex h-11 items-center gap-2 rounded-full border border-stone-200 bg-white/90 px-4 text-sm font-bold text-ink shadow-sm backdrop-blur transition hover:border-stone-300"
      ...
      {open ? (
        <div className="absolute right-0 z-30 mt-2 w-56 overflow-hidden rounded-md border border-stone-200 bg-white shadow-xl">
          {languages.map((item) => (
            <Link
              className="flex items-center justify-between gap-3 border-b border-stone-100 px-4 py-4 text-sm font-bold text-ink last:border-b-0 hover:bg-stone-50"
