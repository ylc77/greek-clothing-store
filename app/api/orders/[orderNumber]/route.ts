...
export async function GET(request:NextRequest,{params}:{params:Promise<{orderNumber:string}>}){
  ...
  const{data:order,error}=await(supabase as any).from("online_orders").select("id,order_number,status,fulfillment_method,payment_method,payment_status,fulfillment_status,subtotal,shipping_total,total,currency,locale,boxnow_locker_id,boxnow_locker_name,boxnow_locker_address,boxnow_locker_postal_code,pickup_code,pickup_ready_at,pickup_expires_at,paid_at,created_at,updated_at,access_token_hash").eq("order_number",orderNumber).maybeSingle();
