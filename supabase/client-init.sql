-- ============================================================
-- Fashion Boutique — 新客户一键数据库初始化
-- 在 Supabase SQL Editor 中执行此文件即可完成所有初始化
-- 可重复执行（幂等），不会报错或产生重复数据
-- ============================================================
...
create policy "Block anon insert" on products for insert with check (false);
drop policy if exists "Block anon update" on products;
create policy "Block anon update" on products for update using (false);
drop policy if exists "Block anon delete" on products;
create policy "Block anon delete" on products for delete using (false);
...
insert into business_settings (business_name, description_en, description_gr)
values ('Fashion Boutique', 'Welcome to our online store.', 'Καλώς ήρθατε στο ηλεκτρονικό μας κατάστημα.')
...
create policy "Block anon insert settings" on business_settings for insert with check (false);
drop policy if exists "Block anon update settings" on business_settings;
create policy "Block anon update settings" on business_settings for update using (false);
drop policy if exists "Block anon delete settings" on business_settings;
create policy "Block anon delete settings" on business_settings for delete using (false);
...
create trigger categories_updated_at before update on product_categories for each row execute function set_updated_at();

-- ============================================================
-- 4. product_subcategories 二级分类表
-- ============================================================
...
create trigger subcategories_updated_at before update on product_subcategories for each row execute function set_updated_at();
