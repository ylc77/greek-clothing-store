...
## 24. Workflow plan versus implemented batch behavior

The store-operations plan describes an eventual batch stocktake workflow but does not assign it an implementation phase; existing stocktake and receiving are still single-Variant transactions. Do not label them atomic multi-Variant batches. See `docs/store-operations-plan-status.md` for the staged implementation boundary. Browser `print`/`afterprint`, Preview deployment success, and an operator's local print confirmation are not hardware acceptance evidence. Physical PT-1509/scanner results must be recorded separately before claiming the complete store workflow is accepted.
