...
Final evidence anchor: annotated tag `audit-v1-6b-local-verified-v4` (the earlier tags remain preserved at their pre-CI-fix commits)
...
| `npm run test:operations-backup-restore` | PASS; four database files and one Storage object restored byte-for-byte; repeated local drills remain under 90 seconds (latest: 82.3 seconds) |
...
- Repeated local recoveries remain under 90 seconds; the latest took 82.3 seconds, below the documented RTO target of four hours. The current RPO target is 24 hours and requires daily database plus Storage backups.
