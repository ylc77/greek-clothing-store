...
Final evidence anchor: annotated tag `audit-v1-6b-local-verified`
...
| `npm run test:operations-backup-restore` | PASS; four database files and one Storage object restored byte-for-byte in 68.3 seconds |
...
- Recovery took 68.3 seconds, below the documented RTO target of four hours. The current RPO target is 24 hours and requires daily database plus Storage backups.
